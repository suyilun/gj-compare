...
const trainCard = (trace) => {
  return (
    <div key={trace.hbaseKey}  className={cardClassName}>
      ...
      <ul>
        <li><span>姓名:</span><span>{trace.name}</span></li>
        <li><span>{trace.trainDate}</span></li>
        <li><span>{trace.departure}</span><span> - </span><span>{trace.terminal}</span></li>
      </ul>
      <img src={`images/life_class/${trace.catg}_life_big.png`}></img>
    </div>
  );
...
const flightCard = (trace) => {
  ...
  if (trace.takeOffStation || trace.terminalStation) {
    liArr.push(<li><span>{trace.takeOffStation}-{trace.terminalStation}</span></li>);
  }
...
const dataTypeIsShow = (options, dataType) => {
  ...
  const filterOptions = options.filter((option) => { return option.value == dataType });
